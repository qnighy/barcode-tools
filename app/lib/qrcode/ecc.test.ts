...
test("1-M one-block decoding example", () => {
  const bits: Bits = {
    bitLength: 208,
    bytes: new Uint8Array([
      0b00010000,
  ...
  const actual = decodeErrorCorrection(
    bits,
    1,
    "M"
  );
...
test("3-H two-blocks decoding example", () => {
  const bits: Bits = {
    bitLength: 560,
    bytes: new Uint8Array([
      0b01000000, // Block 1, Byte 0
...
test("3-H two-blocks decoding with errors example", () => {
  const bits: Bits = {
    bitLength: 560,
    bytes: new Uint8Array([
      0b01000000, // Block 1, Byte 0
  ...
  // Add errors
  bits.bytes[8] ^= 0b11001100;
  bits.bytes[23] ^= 0b00100011;
  bits.bytes[30] ^= 0b11111111;
  bits.bytes[45] ^= 0b11000010;
  const actual = decodeErrorCorrection(
    bits,
    3,
    "H"
